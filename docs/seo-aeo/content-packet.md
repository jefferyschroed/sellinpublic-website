...
- `brief.yaml` defines audience, intent, primary query, AEO question, CTA, and owner. For blog posts, the CTA instructions should require a varied Sell In Public closing CTA that names the company, explains the managed LinkedIn content plus outbound offer, identifies the target buyer, and keeps the CTA body to exactly two sentences.
